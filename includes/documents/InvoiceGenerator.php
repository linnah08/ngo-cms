...
class InvoiceGenerator extends DocumentGenerator {
    ...
    protected function buildHtml(array $order, array $items, array $document): string {
        ...
        // Separate product items from donation items
        ...
        // For invoice total: exclude any donation amounts (invoiced separately)
        $donation_total = array_sum(array_column($donation_items, 'amount_eur'));
        $invoice_total  = $total - $donation_total;
        ...
        $amount_words = self::amountInWordsEur($invoice_total);
        ...
        <!-- Header: title left, doc info right -->
        <table class="doc-header" style="width:100%;margin-bottom:14px;border-bottom:3px solid {$t};">
          <tr>
            <td style="vertical-align:bottom;padding-bottom:8px;">
              <div class="doc-title">ФАКТУРА &nbsp;<span style="font-size:10pt;font-weight:normal;color:{$mu};">Оригинал</span></div>
              <div class="doc-sub">Продажби без ДДС</div>
            </td>
        ...
        <table style="width:100%;margin-bottom:12px;">
          <tr>
            <td style="width:55%;vertical-align:top;">
              ...
              <div style="font-size:8pt;color:{$mu};margin-top:6px;margin-bottom:3px;">Плащане:</div>
              <div style="font-size:8.5pt;">{$pay_label}</div>
            </td>
            <td style="width:45%;vertical-align:top;">
              <table style="width:100%;font-size:9pt;border:1px solid {$br};border-radius:4px;">
                <tr>
                  ...
                  <td style="padding:5px 10px;border-bottom:1px solid {$br};text-align:right;">{$this->fmtEur($invoice_total)}</td>
                </tr>
                <tr>
                ...
                <tr>
                  <td style="padding:7px 10px;font-weight:bold;color:{$t};">Сума за плащане</td>
                  <td style="padding:7px 10px;text-align:right;font-weight:bold;color:{$t};font-size:11pt;">{$this->fmtEur($invoice_total)}</td>
                </tr>
              </table>
            </td>
          </tr>
