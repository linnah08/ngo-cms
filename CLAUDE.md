...
- **lafetki** (`~/Code/lafetki`, github `linnah08/lafetki`) — a git fork of ngo-cms (`upstream` remote). It is the site that proves the update flow before other NGOs are onboarded, so it takes ngo-cms code the way they will: as a tagged release, through the update button in its admin.

Rules:
- **Generic changes are built in ngo-cms first**, then ported to oddminds and reach lafetki with the next release. "Generic" = anything that isn't one site's own copy, branding, theme, content or infra. If a live hotfix has to land on oddminds or lafetki first, port it to ngo-cms in the same session — never leave it for "the next sync".
- **lafetki takes ngo-cms only as a release.** Order: tag a release in ngo-cms → update lafetki from its admin (the server installs the release) → then bring the repo level with the server by merging that tag: `git fetch upstream --tags && git merge v<version>`. Never merge untagged `upstream/main` into lafetki, and never cherry-pick or copy files — untagged code would reach the live site around the update flow it exists to test, and cherry-picks make git lose track of what's shared. The deploy workflow refuses to push a branch that doesn't contain the release the server runs, so after a self-update the tag merge comes before any lafetki deploy.
