...
- **One session per checkout.** A second Claude session working in the same folder at the same time is not safe: sessions stash/reset each other's uncommitted work (test runs then fail for no visible reason), `git add`-ing your own files sweeps up the other's, and `git push` ships their unreviewed commits. If another session is already working in this checkout, run in your own worktree (`git worktree add ../<repo>-wt/<task> -b <task>`), or wait until it is done. Symlink `vendor/`, `node_modules/` and the gitignored `*.config.php` files into the worktree so tests and the dev server work there.
- **Before every push, check what you are actually pushing.** `git fetch && git log --oneline origin/main..HEAD` — if it lists a commit you did not write, do not push; tell the user whose it is and let them decide. The same before `git commit`: stage your files by name, never `git add -A`.
